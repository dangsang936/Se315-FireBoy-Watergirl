...
const HOST_DEATH_TRIGGER_DELAY_SEC: float = 2.0
const PROBE_TIMEOUT_SEC: float = 25.0
const FALL_DEATH_TEST_POS: Vector2 = Vector2(640.0, 880.0)
...
func _run() -> void:
	...
	if _failures.is_empty():
		await _wait_until(func() -> bool:
			return _completed
		, 15.0, "%s probe did not reach LOST state in time." % _probe_role.capitalize())
...
func _connect_joiner() -> void:
	for _attempt: int in CONNECT_RETRY_COUNT:
...
func _spawn_game() -> void:
	...
	_game.set("level_scenes", [LEVEL_ONE_SCENE])
	...
	if _probe_role == "host" and not _death_triggered:
		_death_triggered = true
		_trigger_death_border()
	else:
		_watch_for_lost_state()

func _trigger_death_border() -> void:
	await create_timer(HOST_DEATH_TRIGGER_DELAY_SEC).timeout
	if not is_instance_valid(_network_manager):
		_require(false, "NetworkManager was freed before death-border trigger.")
		return
	print("[host] Teleporting local player into fall death test position %s." % FALL_DEATH_TEST_POS)
	_network_manager.rpc_id(1, "rpc_request_teleport_player", FALL_DEATH_TEST_POS)
	_watch_for_lost_state()

func _watch_for_lost_state() -> void:
	await _wait_until(func() -> bool:
		return is_instance_valid(_game) and int(_game.get("_state")) == int(GameManager.ManagerState.LOST)
	, 8.0, "%s did not enter LOST state after death-border trigger." % _probe_role.capitalize())
	if _failures.is_empty():
		_completed = true
		print("[%s] Multiplayer death-border probe passed." % _probe_role)
